...
- The Python core records bounded operational decisions in a global data-directory log. Its records use fixed codes and local Watchdog UUIDs only; prompts, paths, provider/session IDs, commands, tool output, payloads, exception messages, and tracebacks are excluded. Logging is best effort and cannot change hook output or delivery.
