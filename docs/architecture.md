...
- The Python core records bounded operational decisions in a global data-directory log. Its records use fixed codes, fixed provider names, a shape-bounded `error_type` (exception class name or internal failure code) and field name, and local Watchdog UUIDs only; prompts, paths, provider/session IDs, commands, tool output, payload values, exception messages, and tracebacks are excluded. Logging is best effort and cannot change hook output or delivery.
