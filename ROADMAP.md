...
4. Installer with dry-run/backup/uninstall that preserves existing hooks. Native trust procedures remain mandatory and must use the profile active on the target surface. WD-006 implements explicit JSON file edits; WD-007 adds content capture, enabled by default with project opt-out, plus a credential filter that WD-115 makes an export-time transform. Ordinary tests never install into active provider configuration.
5. Retention of 30/180 days (a disk-budget control; `0` disables time expiry, WD-116), 2 GiB/project, and bounded inbox/logs; pin, reserve, degraded state, and loss counters. Persist the raw provider input locally; the credential filter applies at sanctioned export (WD-115).
