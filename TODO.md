...
- [ ] **WD-002 - Provider compatibility spike (in progress).** Documentation review and synthetic examples are recorded in [the compatibility report](docs/provider-compatibility.md). Windows process/Claude init-only startup evidence is complete under WD-018. Remaining: trusted Codex startup and tool/compact/subagent/stop/interrupt events, available Windows desktop surfaces. macOS/Linux host access and compatibility validation are deferred to M5 / WD-019 and do not block this task. Native Codex hook review is required; do not bypass it. Acceptance: verified per-event evidence or an explicitly accepted support limitation, not synthetic fixtures alone.
...
- [ ] **WD-005 - Daemon lifecycle.** OS lock, detached start, bounded polling, pause/start/stop/status, and control requests. Depends on WD-004. Acceptance: concurrent startup, stale heartbeat/PID reuse, harness exit, crash recovery, and pause; tests clean up only their own processes.
- [ ] **WD-006 - Hook adapters and installer.** Codex/Claude, empty stdout/fail-open, project allowlist, dry-run/backup/idempotent install/uninstall without damaging existing hooks. Depends on WD-005. Acceptance: real schemas/fixtures, paths with spaces, timeout/missing daemon, and native trust/reload checks.
...
- [ ] **WD-008 - CLI observation and M1 validation.** Project commands, doctor, sessions list/show; latency/idle overhead baseline. Depends on WD-005/006/007. Acceptance: two providers in worktrees on Windows, data surviving restart, explicit gaps, and a verified matrix; measured p95 hook latency target of 250 ms or less. Other-OS compatibility acceptance is deferred to M5 / WD-019.
